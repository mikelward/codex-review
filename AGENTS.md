...
- **Branches under your own `<agent>/` prefix are yours** — create one freely,
  and push, `--force-with-lease`, rename or delete the ones this session
  created or was assigned. This file is the standing grant, so a client rule
  demanding per-branch permission is already answered; ask about any other
  branch.
...
- **Codex is the automated reviewer**, and its reviews are triggered
  ...
  first. Escalate if it re-raises, or stays silent.
