...
check "markdown-only diff is docs"        0 "docs_only=true"  FILES="AGENTS.md CHANGELOG.md"
check "code file makes it code"           0 "docs_only=false" FILES="AGENTS.md codex-review.mjs"
check "the manifest is code"              0 "docs_only=false" FILES="action.yml"
check ".gitignore is code like any config" 0 "docs_only=false" FILES=".gitignore AGENTS.md"
check "a shared head never rides the docs lane" 0 "docs_only=false" FILES="AGENTS.md" SHARED_PRS="1 2"
check "a lone foreign head PR never rides the lane" 0 "docs_only=false" FILES="AGENTS.md" SHARED_PRS="2"
check "a run outlived by a force-push is code"  0 "docs_only=false" FILES="AGENTS.md" HEAD_SHA=otherhead
check "a swap after the diff read is code" 0 "docs_only=false" FILES="AGENTS.md" HEAD_AFTER=2
check "a retargeted run is code"           0 "docs_only=false" FILES="AGENTS.md" BASE_REF=other
check "a retarget after the diff read is code" 0 "docs_only=false" FILES="AGENTS.md" BASE_AFTER=2
check "README.md is code (it carries the consumer templates)" 0 "docs_only=false" FILES="README.md"
check "unreadable base is not docs"        0 "docs_only=false" FILES="AGENTS.md" BASE_FAIL=1
check "a twin PR after the diff read is code" 0 "docs_only=false" FILES="AGENTS.md" PULLS_AFTER=2
check "docs on a stacked base still ride the lane" 0 "docs_only=true" FILES="AGENTS.md" EVENT_BASE_REF=claude/lower BASE_REF=claude/lower EVENT_BASE_SHA=basesha
check "a force-pushed stacked base is code" 0 "docs_only=false" FILES="AGENTS.md" EVENT_BASE_REF=claude/lower BASE_REF=claude/lower EVENT_BASE_SHA=basesha BASE_TIP=movedsha
check "an unreadable stacked-base tip is not docs" 0 "docs_only=false" FILES="AGENTS.md" EVENT_BASE_REF=claude/lower BASE_REF=claude/lower EVENT_BASE_SHA=basesha TIP_FAIL=1
check "a base ref with a fragment character still resolves" 0 "docs_only=true" FILES="AGENTS.md" EVENT_BASE_REF=claude/lower#1 BASE_REF=claude/lower#1 EVENT_BASE_SHA=basesha
check "unlistable head PRs are code on a PR event" 0 "docs_only=false" FILES="AGENTS.md" PULLS_FAIL=1
...
check "non-PR events are code"            0 "docs_only=false" FILES="AGENTS.md" GITHUB_EVENT_NAME=push

# --- classify: a dispatched run judges the PR its input names, or nothing
check "dispatch naming a PR classifies it" 0 "docs_only=true"  FILES="AGENTS.md" GITHUB_EVENT_NAME=workflow_dispatch
check "PR-less dispatch is refused"        1 "must name the pull request" FILES="AGENTS.md" GITHUB_EVENT_NAME=workflow_dispatch PR=
check "PR-less dispatch is refused even on main" 1 "must name the pull request" FILES="AGENTS.md" GITHUB_EVENT_NAME=workflow_dispatch PR= GITHUB_REF_NAME=main
check "shared head is refused"             1 "cannot vouch for exactly one" FILES="AGENTS.md" GITHUB_EVENT_NAME=workflow_dispatch SHARED_PRS="1 2"
check "unlistable head PRs are refused"    1 "refusing to report" FILES="AGENTS.md" GITHUB_EVENT_NAME=workflow_dispatch PULLS_FAIL=1
check "dispatch for another PR's commit refused" 1 "must not label" FILES="AGENTS.md" GITHUB_EVENT_NAME=workflow_dispatch HEAD_SHA=otherhead
check "unreadable head refuses the dispatch" 1 "refusing to report" FILES="AGENTS.md" GITHUB_EVENT_NAME=workflow_dispatch HEAD_FAIL=1
...
check "truncated file list is not docs"   0 "docs_only=false" FILES="AGENTS.md" CHANGED=3000
check "files API failure is not docs"     0 "docs_only=false" FILES="AGENTS.md" FILES_FAIL=1
...
check "justified skip with prefixes"      0 "" FILES="AGENTS.md" SUBJECTS="docs: Fix a typo" CLASSIFY=success RESULTS="$ALLSKIP"
check "build prefix fails the lane"       1 "lacks a housekeeping prefix" FILES="AGENTS.md" SUBJECTS="build: Note the runner major" CLASSIFY=success RESULTS="$ALLSKIP"
check "docs prefix rides the lane"        0 "" FILES="AGENTS.md" SUBJECTS="docs: Note the runner major" CLASSIFY=success RESULTS="$ALLSKIP"
check "merge commits are exempt"          0 "" FILES="AGENTS.md" SUBJECTS="2:Merge branch 'main' into x" CLASSIFY=success RESULTS="$ALLSKIP"
...
check "a shared head cannot earn a gate"  1 "cannot vouch for exactly one" FILES="AGENTS.md" SUBJECTS="docs: x" CLASSIFY=success RESULTS="$ALLSKIP" SHARED_PRS="1 2"
check "a foreign PR's head cannot earn a gate" 1 "cannot vouch for exactly one" FILES="AGENTS.md" SUBJECTS="docs: x" CLASSIFY=success RESULTS="$ALLSKIP" SHARED_PRS="2"
check "gate refuses a run outlived by a force-push" 1 "own run owns the verdict" FILES="AGENTS.md" SUBJECTS="docs: x" CLASSIFY=success RESULTS="$ALLSKIP" HEAD_SHA=otherhead
check "skip after a mid-read swap is refused" 1 "refusing the skip" FILES="AGENTS.md" SUBJECTS="docs: x" CLASSIFY=success RESULTS="$ALLSKIP" HEAD_AFTER=2
check "gate refuses a retargeted run"     1 "own run owns the verdict" FILES="AGENTS.md" SUBJECTS="docs: x" CLASSIFY=success RESULTS="$ALLSKIP" BASE_REF=other
check "skip after a mid-read retarget is refused" 1 "refusing the skip" FILES="AGENTS.md" SUBJECTS="docs: x" CLASSIFY=success RESULTS="$ALLSKIP" BASE_AFTER=2
check "skip after a post-lint swap is refused" 1 "moved while the gate was reading" FILES="AGENTS.md" SUBJECTS="docs: x" CLASSIFY=success RESULTS="$ALLSKIP" HEAD_AFTER=4
check "skip after a late twin PR is refused"   1 "refusing the skip" FILES="AGENTS.md" SUBJECTS="docs: x" CLASSIFY=success RESULTS="$ALLSKIP" PULLS_AFTER=2
check "gate refuses a force-pushed stacked base" 1 "own run owns the verdict" FILES="AGENTS.md" SUBJECTS="docs: x" CLASSIFY=success RESULTS="$ALLSKIP" EVENT_BASE_REF=claude/lower BASE_REF=claude/lower EVENT_BASE_SHA=basesha BASE_TIP=movedsha
check "docs on a stacked base pass the gate" 0 "" FILES="AGENTS.md" SUBJECTS="docs: x" CLASSIFY=success RESULTS="$ALLSKIP" EVENT_BASE_REF=claude/lower BASE_REF=claude/lower EVENT_BASE_SHA=basesha
...
check "truncated commit list fails lint"  1 "Commit list incomplete" FILES="AGENTS.md" SUBJECTS="docs: x" NCOMMITS=300 CLASSIFY=success RESULTS="$ALLSKIP"
